...
class _StatisticsPageState extends State<StatisticsPage> {
  Future<String> _getDaysUntilExam() async {
    ...
    if (!userDoc.exists || userDoc['examDate'] == null) {
      return 'No exam date set';
    }

    final String examDateString = userDoc['examDate'];
    final DateTime examDate = DateTime.parse(examDateString);
    final DateTime now = DateTime.now();
    final int daysRemaining = examDate.difference(now).inDays + 1;
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, userSnapshot) {
          ...
          final String examDateString = userData['examDate'] ?? '';
          final DateTime examDate = DateTime.parse(examDateString);
          final DateTime now = DateTime.now();
          final int daysRemaining = examDate.difference(now).inDays + 1;
          final String daysUntilExam = '$daysRemaining dni';
          ...
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              int totalCorrectAnswers = currentWeekResults.fold(
                  0,
                  (sum, result) =>
                      sum + (result['correctAnswers'] as int ?? 0));
              double averageScore =
                            ...
                            Container(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  final entry =
                                      topMissedQuestions.elementAt(index);
                                  return Column(
                                    crossAxisAlignment:
